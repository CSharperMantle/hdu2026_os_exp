...
+ 了解 Linux 操作系统的体系结构和运行机制。
+ 掌握内核源码的获取、配置、编译流程，并能够替换操作系统运行自己的内核。
+ 深入理解 Linux 系统调用的工作原理，掌握向 Linux 内核添加自定义系统调用的步骤及相关数据结构的修改方法。
+ 熟悉 Linux 环境下的 C 语言编程思维以及用户态与内核态数据交互的方法。
