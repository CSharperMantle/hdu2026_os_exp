#!/bin/bash
# test_mychat_mq_stress_concurrent_send.sh
# Tests: MQ sync correctness under concurrent senders + queue backpressure.
# Three clients send simultaneously; queue capacity (mq_maxmsg=4) causes
# blocking mq_send backpressure. All 30 messages must eventually arrive.
# Usage: test_mychat_mq_stress_concurrent_send.sh path/to/mychat
...
# mq_send is blocking; clients will queue behind each other when host MQ fills.
{
...
# ── All messages must arrive (allow up to 15 s for MQ backpressure) ──────────
