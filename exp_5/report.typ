...
#paragraph([设备抽象与盘上组织])[
  实验中的文件系统采用分层设备抽象将存储介质的物理细节与核心逻辑解耦。如@figure:phys-blocks 所示，物理块设备抽象层以后端支持的一次读写大小为单位提供最基本的读写功能；上层逻辑块设备适配器将多个物理块合并为更大的逻辑块供文件系统核心使用，逻辑块大小须为物理块大小的 $2^n$ 倍。这种分层设计使得文件系统核心逻辑可在内存模拟、磁盘文件镜像、块设备后端等不同底层设备之间复用，而无需修改核心代码。
]
...
本实验实现使用 FAT 文件系统的文件名命名规则，存储大写 ASCII、`.` 与空格分隔的定长记录。因此需对用户输入的路径进行规格化，包括拆分扩展名、空格填充、拒绝过长或不符合要求的文件名等。本实验为了支持 Linux 上常见的“dotfile”文件名，如 `.git` 等，对 FAT 命名规则进行扩展，允许在扩展名存在的前提下基础名为空。
...
libmyfs 用 `DirEntryLoc` 结构体标识文件、目录节点，其实际上是父目录起始簇号与 FCB 槽位序号组成的二元组，唯一确定盘上任一目录项在其目录的簇链中的逻辑位置；根节点则通过 `NodeMeta` 中 `loc` 字段的 `None` 变体表示。后续挂接 FUSE 所需的索引节点编号不由 libmyfs 产生，由用户程序自行从 `DirEntryLoc` 可逆编码得到。

== 实现简单 Shell：myfs_shell

`myfs_shell` 的设计目标是以最小的工程量暴露文件系统的完整功能，用于验证与调试。核心设计决策是路径解析的单入口模式：所有对文件系统对象的访问统一经过 `resolve_target` 函数，该函数从根目录开始向下遍历路径组件，逐步调用 `lookup` 确认每个中间目录的存在，最终返回包含 `DirEntryLoc`、已缓存的 `Fcb` 与规范化路径的 `ResolvedTarget` 枚举，调用方可按需取用。路径遍历过程与 `..` 的父目录回溯在同一趟中完成，用户侧则无需关心路径是绝对还是相对形式。
...
FUSE 围绕索引式文件系统设计，而本实验中实现的类 FAT16 文件系统不存在索引节点，因此需要在应用中为 FUSE 交互手动构造稳定、唯一的“伪”索引节点标识，并能够高效相互转换。本实验实现选择将 `DirEntryLoc` 编码为 `u64`：高 32 位记录父目录起始簇号、低 32 位记录槽位索引，`0x1` 保留给根节点，与 Linux 标准一致。

FUSE 协议要求实现带有偏移量的随机 I/O，而已有为 Shell 提供的接口则采用基于游标的顺序 I/O。由于底层的 FAT 簇链读取函数已经支持随机读写，只需将其以不同方式挂接到外部接口即可，可以快速提供两套正交的读写接口。

`myfs` FUSE 用户态程序提供了两种启动方式。`-M` 在内存中创建新格式化后的文件系统并挂载，适用于快速演示。`-i` 则挂载已有的格式化镜像文件，展示文件系统的持久化能力。
...
本实验实现的 `myfs_shell` 交互式 Shell 支持完整的目录与文件操作命令集。该 Shell 首先格式化一个运行在内存后端上的 libmyfs，在其上支持下列功能：

- 目录穿越：`pwd`、`cd`
- 列出当前目录内容：`ls`
- 目录操作：`mkdir`、`rmdir`
- 文件操作：`create`、`rm`、`open`
- 文件描述符操作：`close`、`read`、`write`、`seek`、`truncate`、`openfiles`
- 脏 FAT 写回：`sync`
- 数据检视：`fat`、`stat`
- 其他：`help`、`exit`

@figure:shell-demo 中演示了创建目录、创建文件、写入数据、读出数据、修改游标位置、列出目录等操作，说明 myfs_shell 能够完成实验要求的功能。

#img(
  image("assets/shell_demo.png", width: 65%),
  caption: [交互式 Shell 运行演示],
) <figure:shell-demo>

本实验实现的 `mkfs.myfs(1)` 格式化程序可以根据用户提供的逻辑块大小、磁盘逻辑块数量、簇大小等参数对给定的镜像文件进行格式化；格式化后的镜像可使用同样是本实验实现的 `myfs(1)` FUSE 用户态程序挂载。挂载后的文件系统可被标准 Linux 工具直接访问，如使用 Bash `cd(1p)` 进入挂载的目录树，在该目录下使用 `ls(1)` 列出当前目录下的详细信息，使用 `mkdir(1)`、`touch(1)` 创建目录与文件， 使用 Bash 重定向、`truncate(1)` 等工具修改任意文件，使用 `cat(1)` 读取文件。其中，`ls(1)` 可正确显示 FUSE 用户态程序响应的权限与所有者信息，能够正确读取修改时间与文件大小。如@figure:fuse-demo 所示，在挂载点下创建了名为“SUBDIR2”的子目录与名为“1.BIN”的文件，并向其先后写入“Hello”字符串与从 `/dev/urandom` 中获取的 10240 个随机字节，并使用 `ls(1)`、`cat(1)` 与 `wc(1)` 检查写入内容的正确性。
...
在使用 `fusermount(3)` 卸载磁盘后，可以使用 `xxd(1)` 检查磁盘镜像的落盘内容。如@figure:xxd-dump 所示，`0x0` 偏移处的引导扇区、`0x400` 偏移处的 FAT#sub[1]、`0x800` 处的 FAT#sub[2]、`0xc00` 处根目录下的两个 FCB 结构均正确并符合预期。

#img(
  grid(
    columns: (1fr,),
    gutter: 1em,
    image("assets/xxd_boot_sector.png"),
    image("assets/xxd_fat_1.png"),
    image("assets/xxd_fat_2.png"),
    image("assets/xxd_fcb_root.png"),
  ),
  caption: [磁盘镜像 myfs.img 十六进制转储内容],
) <figure:xxd-dump>
